...
public class ProjectService : IProjectService
{
    ...
    public async Task<bool> DeleteAsync(int id, int userId)
    {
        var project = await _context.Projects.FindAsync(id);
        var entrepreneurProfile = await _context.EntrepreneurProfiles
            .Include(ep => ep.User) // Include User to update counter and block status
            .FirstOrDefaultAsync(ep => ep.UserId == userId);
        ...
        bool isPublished = project.Status != "Draft";

        if (isPublished)
        {
            // Refund all investments back to investors
            var investments = await _context.Investments
                .Where(i => i.ProjectId == id && (i.Status == "Pending" || i.Status == "Confirmed"))
            ...
            foreach (var investment in investments)
            {
                ...
                if (wallet != null)
                {
                    ...
                    {
                        // Money was locked. Return to available balance.
                        wallet.Balance += investment.Amount;
                        wallet.LockedAmount -= investment.Amount;
                    }
                    else if (investment.Status == "Confirmed")
                    {
                        // Money was permanently deducted. Add back to available balance.
                        wallet.Balance += investment.Amount;
                    }
                    ...
                    var transaction = new WalletTransaction
                    {
                        ...
                        Description = $"Refund for deleted project: {project.Title}",
                        CreatedAt = DateTime.UtcNow,
            ...
            // Increment deleted projects counter and apply automatic block
            if (entrepreneurProfile.User != null)
            {
                entrepreneurProfile.User.DeletedPublishedProjects++;
                if (entrepreneurProfile.User.DeletedPublishedProjects >= 2)
                {
                    entrepreneurProfile.User.IsBlocked = true;
                }
            }
        }
