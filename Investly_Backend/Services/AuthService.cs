...
public class AuthService : IAuthService
{
    ...
    public async Task<LoginResponse?> LoginAsync(LoginRequest dto)
    {
        ...
        // Swagger exposes both email and phone login. Prefer email when it is supplied.
    ...
    public async Task<LoginResponse?> RegisterAsync(RegisterRequest dto)
    {
        ...
        var user = new User
        {
            ...
            FirstName = !string.IsNullOrWhiteSpace(dto.FirstName) ? dto.FirstName : dto.Name,
        ...
        // Every account gets User. Investor/Entrepreneur can be requested by the client.
        // Admin is intentionally not allowed through public registration.
        var requestedRole = NormalizePublicRole(dto.Role);
        var roleNames = new[] { "User", requestedRole }.Where(r => !string.IsNullOrEmpty(r)).Distinct().ToList();
        ...
        await _notifications.CreateAsync(
            user.UserId,
            "system",
            "مرحباً بك في Investly",
            "Welcome to Investly",
            "تم إنشاء حسابك بنجاح.",
            "Your account was created successfully. Explore projects and start investing with confidence.",
            null,
            null);
    ...
    public async Task<bool> ChangePasswordAsync(int userId, ChangePasswordRequest dto)
    {
        ...
        var currentPassword = !string.IsNullOrWhiteSpace(dto.OldPassword) ? dto.OldPassword : dto.CurrentPassword;
